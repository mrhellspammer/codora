...
const Courses = () => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch("https://codora.onrender.com/courses/all", {
        headers: {
  ...
  const handleAddCourse = async (e) => {
    ...
    try {
      const res = await fetch("https://codora.onrender.com/courses/add", {
        method: "POST",
  ...
  const handleDeleteCourse = async (id) => {
    try {
      const res = await fetch(`https://codora.onrender.com/courses/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error(await res.text());
  ...
  const handleUpdateCourse = async (e) => {
    ...
    try {
      const res = await fetch(`https://codora.onrender.com/courses/${editingCourse.id}`, {
        method: "PUT",
        headers: {
    ...
    <div className="p-20 w-full bg-gradient-to-br from-gray-100 to-slate-200 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {courses.map((course) => (
          ...
          >
            <img
              src={`https://codora.onrender.com${course.imagePath}`}
              alt={course.title}
